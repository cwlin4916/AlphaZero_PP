"""Configuration for ExplicitSurfaceDerivationGame on the Doors environment.

Identical to DoorsSurfaceDerivationConfig but uses the grammar-based
ExplicitSurfaceDerivationGame instead of the bitmask-based
SurfaceDerivationGame. Drop-in swap — same network, same agent, same trainer.
"""
...
@dataclass
class DoorsExplicitSurfaceDerivationConfig(MetaConfig):
    """Configuration for ExplicitSurfaceDerivationGame on Doors.

    The grammar-based game has the same action space (2K+1) and episode
    length (2K+1) as the bitmask game. The only difference is that legal
    actions come from SurfaceCFG productions instead of bitmask logic.
    """
    ...
    def __init__(self, num_rooms: int = 3):
        ...
        self.game = CoreGameConfig(
            game_cls=ExplicitSurfaceDerivationGame,
            kwargs={
                "num_rooms": num_rooms,
                "locs_per_room": 2,
        ...
        self.run = RunConfig(
            ...
            plot_every=5,
            plot_path="doors_explicit_surface_training_metrics.png",
        )

    def build(self):
        """Build ExplicitSurfaceDerivationGame, network, agent, trainer, evaluator."""
        gk = self.game.kwargs
